<template>
    <div>
        ...
        <el-table :data="tableData" style="width: 100%">
            <el-table-column prop="id" label="用户ID" width="80" flex="left" />
            ...
            <el-table-column label="情绪评分">
                <template #default="scope">
                    <el-rate v-model="scope.row.moodScore" :max="10" disabled text-color="#ff9900" />
                </template>
            </el-table-column>
...
const handleSearch = async (formData) => {
    ...
    const data = await emotionalPage(params)
...
const handleDelete = (row) => {
    ElMessageBox.confirm('确认删除该记录吗？', '确认删除', {
